...
function loadFormState() {
    ...
    if(state) {
        ['liga','loteFornada','forno','polegada','comprimento'].forEach(id => {
...
function limparTudo() {
    ...
    if (a) {
        ...
        unlockFields();
        ['loteFornada', 'liga', 'forno', 'polegada', 'comprimento'].forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
        if(typeof scaleConnected !== 'undefined' && !scaleConnected) document.getElementById('pesoBruto').value = '';
        document.getElementById('tipo').value = 'Lingote'; document.getElementById('quantidade').value = '0';
        toggleCamposTarugo(); showToast("Tudo limpo.", "success");
    }
    saveFormState();
}
...
function gerarEtiqueta(i) { 
    const pesoExibicao = i.peso || i.bruto;
        ...
        <div style="padding:2mm; display:block !important; height:auto !important; box-sizing:border-box; page-break-after: avoid !important; font-weight: bold !important; color: black;">
            ...
            <div style="font-size:10px;line-height:1.4;display:flex;justify-content:space-between;margin-bottom:2px;">
                <div style="text-align:left;width:48%">
                    <div>Liga: ${i.liga}</div>
                    <div style="margin-top:2px">Lote: ${i.lote}</div>
                    <div style="margin-top:2px">Tipo: ${i.tipo.toUpperCase()}</div>
                </div>
                <div style="text-align:right;width:50%">
                    ${i.tipo === 'Tarugo' ? `
                    <div>Fornada: ${i.forno}</div>
                    <div style="margin-top:2px">Polegada: ${i.polegada}"</div>
                    <div style="margin-top:2px">Compr.: ${i.comprimento}m</div>` : ''}
                </div>
            </div>
            <div style="text-align:center;border-top:2px dashed #000;border-bottom:2px dashed #000;padding:3mm 0;margin:2mm 0;">
                ...
                <div style="font-size:16px;">Quantidade: ${i.qtd}</div>
            </div>
            <div style="text-align:center;margin-top:2mm;">
                <div style="font-family:'Libre Barcode 128',cursive;font-size:32px;line-height:1;font-weight:normal !important;">*${i.lote}*</div>
                <div style="margin-top:2mm;border-top:1px solid black;padding-top:1mm;font-size:8px;">
                    DATA: ${i.data} | HORA: ${i.hora} | RESP.: ${i.responsavel.toUpperCase()}
                </div>
            </div>
        </div>`; 
...
function gerarRelatorio(){ 
    ...
    const filename = `RPLT_${new Date().toLocaleDateString('pt-BR').replace(/\//g,'-')}_${horaStr}_${db[0].liga}_${db[0].lote.replace(/[\/\\]/g,'-')}_${qtdTotal}un_${tb.toFixed(2)}kg`; 
    const conteudoHTML = `
...
window.onload = () => { 
    ...
    if(d.length > 0) {
        ...
        toggleCamposTarugo(); lockFields(); showToast("Atenção: Pesagem em andamento restaurada.", "info");
    } 
    renderHistory(); 
